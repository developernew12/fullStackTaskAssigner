...
const AdminMeetings = () => {
  ...
  return (
    <div
      className={`${darkMode ? styles.darkContainer : styles.mainContainer}`}
    >
      <h2 className={styles.heading}>All Scheduled Meetings</h2>
      {meetings.length === 0 ? (
        ...
        <div className={styles.gridContainer}>
          {meetings.map((meeting) => (
            <div key={meeting._id} className={styles.meetingCard}>
              <h1 className={styles.topic}>{meeting.topic}</h1>
              <p>
                <strong>Room ID:</strong> {meeting.roomId}
              </p>
              <p>
              ...
              <p>
                <strong>Users:</strong>{" "}
                {meeting.assignedUsers.map((u) => u.name).join(", ")}
              </p>
              <p>
                <strong>Time:</strong>{" "}
